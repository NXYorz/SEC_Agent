...
class Algorithm:
    ...
    def learn(self, list_sample_data):
        ...
        if now - self.last_report_monitor_time >= 60:
            ...
            self.logger.info(
                ...
                f"policy_obj:{results['policy_objective']} "
    ...
    def _compute_loss(
        ...
        # 进一步限制 log-ratio，抑制极端样本导致的策略更新震荡。
        log_ratio = (new_log_prob - old_log_prob).clamp(-2.0, 2.0)
        ratio = log_ratio.exp()
        ...
        clipped_objective = torch.minimum(policy_loss1, policy_loss2)
        # Dual-clip PPO：负优势样本上再做一道上限，降低 policy_loss 抖动。
        dual_clip = 3.0
        policy_objective = torch.where(
            adv < 0.0,
            torch.maximum(clipped_objective, dual_clip * adv),
            clipped_objective,
        ).mean()
        # PPO 训练时我们最小化 loss，因此策略项写成 objective 的相反数。
        # 这意味着 policy_loss 为负值是正常现象（objective 越大，loss 越小）。
        ...
        return total_loss, [value_loss, policy_loss, policy_objective, entropy_loss]
