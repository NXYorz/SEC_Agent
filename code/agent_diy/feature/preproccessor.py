...
def check_monsterAndHero(monster , hero , env_info):
    if monster["hero_l2_distance"] > 0 and env_info["flash_cooldown"] == 0:
        return False
    if Dis(monster , hero) > 32 and env_info["flash_cooldown"] == 0:
        return False
...
def Greddy(isDanger , monsters , organs):
    if isDanger == 0:
        return 1
    if organs[0]["hero_relative_direction"] != mosnters[0]["hero_relative_direction"] and organs[0]["hero_relative_direction"] != mosnters[1]["hero_relative_direction"]:
        return 1
    return 0
...
class Preprocessor:
    ...
    def feature_process(self, env_obs, last_action):
        ...
        flash_ready = 0
        if env_info["flash_cooldown"] > 0:
            flash_ready = 1
        buff_remain_norm = _norm(env_info["buff_refresh_time"], MAX_BUFF_DURATION)
        score = env_info["total_score"]
        frame_id = frame_state["frame_no"]
        isStop = 0
        if hero_pos["x"] == last_x and hero_pos["z"] == last_z:
            isStop = 1
        last_x = hero_pos["x"]
        last_z = hero_pos["z"]

        area_x , area_z = get_area(hero_pos["x"] , hero_pos["z"])
        if area_x == last_area_x and area_z == last_area_z:
            cycleStep += 1
        else:
            cycleStep = 0
            last_area_x = area_x
